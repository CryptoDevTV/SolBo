...
namespace SolBo.Shared.Rules.Mode.Production.Exchange
{
    public class KucoinAccountExchangeRule : IExchangeRule
    {
        ...
        public IRuleResult RuleExecuted(Solbot solbot)
        {
            ...
            {
                var accountType = accountInfo.Data.FirstOrDefault(a => a.Type == KucoinAccountType.Trade);

                if (!(accountType is null))
                {
                    solbot.Communication.AvailableAsset = new AvailableAssetMessage
                    {
                        Quote = accountType.Available
                    };

                    baseMsg = $"{solbot.Communication.Symbol.BaseAsset}:{string.Empty}";
                    quoteMsg = $"{solbot.Communication.Symbol.QuoteAsset}:{accountType.Available}";
