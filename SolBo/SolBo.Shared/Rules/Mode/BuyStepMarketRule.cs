        ...
        public IRuleResult RuleExecuted(Solbot solbot)
        {
            ...
            return new MarketRuleResult()
            {
                ...
                Message = result.PercentChanged < 0
                    ? $"{OrderName} => Price ({solbot.Communication.Price.Current}) increased from the average ({solbot.Communication.Average.Current}) by {Math.Abs(solbot.Communication.Buy.Change)}%"
                    : $"{OrderName} => Price ({solbot.Communication.Price.Current}) has fallen from the average ({solbot.Communication.Average.Current}) by {Math.Abs(solbot.Communication.Buy.Change)}%"
            };
        }
    }
}